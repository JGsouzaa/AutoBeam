...
class oBeam():
    ...
    #Need to colect info, and set as object properties
    def __init__(self, barNumber):
        self.number = barNumber
        self.materialData = get_materialInfo(self.number)
        self.sectionData = get_sectionInfo(self.number)

    def get_materialInfo(self,barNumber):
        dMaterialData = {}
        data = IRobotMaterialData(IRobotLabel(IRobotBar(bars.Get(barNumber).GetLabel(8)).Data))
        dMaterialData["materialData"] = data
        dMaterialData["name"] = data.get_Name()
        dMaterialData["fck"] = data.get_RE()
        dMaterialData["elastModulus"] = data.get_E()
        dMaterialData["poisModulus"] = data.get_Kirchoff()
        dMaterialData["specWeight"] = data.get_RO()
        dMaterialData["termicExpCoef"] = data.get_LX()
        dMaterialData["dumpCoef"] = data.get_DumpCoef()
        return dMaterialData

    def get_sectionInfo(self,barNumber):
        dSectionInfo = {}
        dSectionInfo["heigth"] = get_heigth(barNumber)
        dSectionInfo["width"] = get_width(barNumber)
        


    def convertUnits(self):

    def get_concreteType(self):
